...
INT WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nShowCmd)
{
	...
	while (1)
	{
		...
		// 注意，每一次绘制都是取的当前的设置，比如下面设置的颜色
		// 画线，不是GL_LINE
		glLineWidth(4.0f); // 线的宽度
		glBegin(GL_LINES);
		glColor4ub(255, 0, 0, 255);	// 设置一个颜色
		...
		// 环线，第一个点连向最后一个点，又从最后一个点连接到第一个点
		glBegin(GL_LINE_LOOP);
		glColor4ub(0, 255, 0, 255);	// 设置一个颜色
		glVertex3f(0.0f, 0.0f, -10.0f);
		glVertex3f(5.0f, 0.0f, -10.0f);
		glVertex3f(5.0f, -2.0f, -10.0f);
		glEnd();

		// 首尾相接，但是最后一个点不会再去连接第一个点
		glBegin(GL_LINE_STRIP);
		glColor4ub(0, 0, 255, 255);	// 设置一个颜色
		glVertex3f(0.0f, 0.0f, -10.0f);
		glColor4ub(0, 0, 255, 255);	// 设置一个颜色
